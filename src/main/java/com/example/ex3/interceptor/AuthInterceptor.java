...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (user == null && !requestURI.startsWith("/guestbook") && !requestURI.equals("/login")  && !requestURI.equals("/") && !requestURI.equals("/signup")  && !requestURI.equals("/otp") && !requestURI.startsWith("/map") && !requestURI.startsWith("/qna") && !requestURI.startsWith("/st") && !requestURI.startsWith("/ed") && !requestURI.startsWith("/pk") && !requestURI.startsWith("/mg") && !requestURI.startsWith("/etc") && !requestURI.startsWith("/posts")){
            ...
            // 사용자 인증 정보가 있는 경우 "/posts" 요청에 대해서는 리다이렉트를 시키지 않음
            return true;
        }

        // 예외 페이지 설정
        if ( requestURI.equals("/") || requestURI.equals("/list") || requestURI.equals("/signup") || requestURI.equals("/otp") || requestURI.startsWith("/map") || requestURI.startsWith("/qna") || requestURI.startsWith("/st") || requestURI.startsWith("/mg") || requestURI.startsWith("/ed") || requestURI.startsWith("/pk") || requestURI.startsWith("/etc")){
            return true;
        }
